...
def test_each_horse_gets_a_factual_paragraph():
    ...
    assert "2 performances sont documentées" in paragraph
    assert "sur 2100 m" in paragraph
    assert "ferrure D4" in paragraph
    assert "potentiel caché" in paragraph.lower()


def test_no_evidence_is_neutral_and_never_ranked():
    meeting = Meeting(id=1, race_date=date.today(), code="R1", track="Deauville")
    race = Race(id=1, meeting=meeting, meeting_id=1, code="R1C1", name="x", scheduled_at=datetime.now(), discipline="Plat", distance_m=2000)
    runner = Runner(id=1, race_id=1, number=14, horse_name="DEBUTANTE", sex="FEMELLES", age=3, weight_kg=55, draw=1)
    runner.raw = {"history_status": "history_incomplete"}
    card = score_race(race, [runner])[1]
    assert card.performance == 50
    assert card.placed == 50
    assert card.breakdown["ranking_eligible"] is False
    assert "Conclusion" in card.breakdown["analysis_text"]
    assert "COURSE_" not in card.breakdown["analysis_text"]


def test_unavailable_history_is_not_presented_as_a_positive_signal():
    meeting = Meeting(id=1, race_date=date.today(), code="R1", track="Deauville")
    race = Race(id=1, meeting=meeting, meeting_id=1, code="R1C1", name="x", scheduled_at=datetime.now(), discipline="Plat", distance_m=2000)
    runner = Runner(id=1, race_id=1, number=4, horse_name="SOURCE INDISPONIBLE")
    runner.raw = {"history_status": "unavailable"}
    card = score_race(race, [runner])[1]
    assert card.breakdown["evidence_label"] == "Historique indisponible"
    assert "n’a pas abouti" in card.breakdown["analysis_text"]
