...
def test_normalize_pmu_program_and_runners():
    ...
    runners=PmuProvider.normalize_runners({"participants":[{
        ...
        "handicapPoids":550,"handicapValeur":350,"positionDepart":2
    }]})
    runner=runners["data"]["partants"][0]
    assert runner["num"]==4
    assert runner["jockey_driver"]=="A. TEST"
    assert runner["np"] is False
    assert runner["poids"] == 55
    assert runner["valeur"] == 35
    assert runner["position_depart"] == 2


def test_galop_stall_is_not_copied_into_start_position():
    runner = PmuProvider.normalize_runners({"participants": [{
        "numPmu": 14, "nom": "NATIVE DE BOZOULS", "placeCorde": 1,
        "handicapPoids": 550,
    }]})["data"]["partants"][0]
    assert runner["corde"] == 1
    assert runner["position_depart"] is None


def test_detailed_performances_keep_objective_opponents_only():
    payload = {
        "participants": [{
            "numPmu": 4,
            ...
            "coursesCourues": [{
                ...
                "nomPrix": "PRIX DES LIGNES",
                "discipline": "PLAT",
                "allocation": 42000,
                "distance": 2000,
                "nbParticipants": 12,
                "participants": [
                    {"nomCheval": "CHEVAL TEST", "itsHim": True, "place": {"place": 1, "statusArrivee": "PLACE"}, "reductionKilometrique": 7420},
                    {"nomCheval": "RIVAL SOLIDE", "itsHim": False, "place": {"place": 2, "statusArrivee": "PLACE"}, "reductionKilometrique": 7440, "rapportProbable": 3.2},
                    {"nomCheval": "RIVAL NON PLACE", "itsHim": False, "place": {"place": None, "statusArrivee": "NON_PLACE"}},
                ],
                "commentaire": "Très belle chance",
            }],
        }],
    }
    normalized = PmuProvider.normalize_detailed_performances(payload)
    runner = normalized["data"]["runners"][0]
    row = runner["historique"][0]
    assert runner["number"] == 4
    assert row["position"] == 1
    assert row["allocation_eur"] == 42000
    assert row["adversaires"][0]["horse_name"] == "RIVAL SOLIDE"
    assert row["adversaires"][1]["result_status"] == "NON_PLACE"
    assert row["reduction_km"] == 74.2
    flat = str(normalized).lower()
    assert "rapport" not in flat
    assert "belle chance" not in flat
