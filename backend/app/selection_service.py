...
def _reason(kind: str, item: dict, score: float) -> str:
    ...
    evidence = f"{n} course{'s' if n != 1 else ''} documentée{'s' if n != 1 else ''}"
    if kind == "horse":
    ...
    return (
        f"Indice {score:.0f}/100 après examen de {evidence} : {basis}. "
        f"Confiance documentaire {confidence:.0f}/100.{suffix}"
    )
...
def choose(items: Iterable[dict], kind: str) -> dict | None:
    ...
    documented = [item for item in items if _sample_size(item) > 0]
