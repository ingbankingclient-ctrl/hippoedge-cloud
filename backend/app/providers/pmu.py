...
def _scaled_decimal(value: Any, *, threshold: float) -> Any:
    """Decode PMU decimal fields that are sometimes transported as tenths.

    The public feed represents values such as 55 kg as ``550`` on some race
    cards, while other cards already expose ``55``.  Keeping the threshold
    deliberately high lets the adapter accept both representations without
    changing legitimate mounted-race weights.
    """
    if value in (None, ""):
        return None
    try:
...
def _result_position(value: Any) -> tuple[int | None, bool, str | None]:
    place = value.get("place") if isinstance(value, dict) else value
    status = value.get("statusArrivee") if isinstance(value, dict) else None
    try:
        position = int(place) if place not in (None, "") else None
    except (TypeError, ValueError):
        position = None
    status_text = str(status or "").upper() or None
    disqualified = bool(status_text and any(token in status_text for token in ("DISQUAL", "DISTANC")))
    if disqualified:
        position = None
    return position, disqualified, status_text


def _performance_date(value: Any, offset_ms: Any = 0) -> str | None:
    if isinstance(value, (int, float)):
        try:
            offset = float(offset_ms or 0) / 1000
            return (datetime.fromtimestamp(value / 1000, timezone.utc) + timedelta(seconds=offset)).date().isoformat()
...
class PmuProvider(RacingProvider):
    ...
    def normalize_runners(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for p in raw:
            ...
            runners.append({
                ...
                "poids": _scaled_decimal(
                    p.get("handicapPoids") or p.get("poidsConditionMonte"),
                    threshold=200,
                ),
                "corde": p.get("placeCorde"),
                "valeur": _scaled_decimal(p.get("handicapValeur"), threshold=100),
                ...
                # A stall/draw number in the galop is not an autostart position.
                # They are kept in distinct fields so the explanation does not
                # repeat "position 1" and "corde 1" for the same fact.
                "position_depart": p.get("positionDepart") or p.get("numeroAutostart"),
    ...
    async def get_horse_history(
        ...
        return await self.history_client.get_history(
            horse_name or horse_id,
            discipline,
            horse_id=horse_id,
            race_date=race_date,
        )

    async def get_historical_course(self, course_id: str | int) -> dict[str, Any]:
        """Read one exact past-race field from the public Geny complement."""
        return await self.history_client.get_course_participants(course_id)

    @staticmethod
    def normalize_detailed_performances(payload: dict[str, Any]) -> dict[str, Any]:
        """Map PMU's factual race-line feed without importing market fields.

        Each former race contains the current horse plus the published finishers
        present in the feed.  Their names and objective results form the graph;
        comments, odds and predictions are deliberately not addressed.
        """
        root = _root_payload(payload)
        raw_runners = root.get("participants") or payload.get("participants") or []
        runners: list[dict[str, Any]] = []
        for raw_runner in raw_runners if isinstance(raw_runners, list) else []:
            if not isinstance(raw_runner, dict):
                continue
            number = raw_runner.get("numPmu") or raw_runner.get("numero") or raw_runner.get("num")
            horse_name = raw_runner.get("nomCheval") or raw_runner.get("nom")
            history: list[dict[str, Any]] = []
            courses = raw_runner.get("coursesCourues") or raw_runner.get("courses") or []
            for course in courses if isinstance(courses, list) else []:
                if not isinstance(course, dict):
                    continue
                race_date = _performance_date(course.get("date"), course.get("timezoneOffset"))
                if not race_date:
                    continue
                field = course.get("participants") or []
                own: dict[str, Any] | None = None
                for participant in field if isinstance(field, list) else []:
                    if not isinstance(participant, dict):
                        continue
                    if participant.get("itsHim") is True:
                        own = participant
                        break
                    if horse_name and str(participant.get("nomCheval") or "").strip().casefold() == str(horse_name).strip().casefold():
                        own = participant
                if own is None:
                    continue
                own_position, own_dq, own_status = _result_position(own.get("place"))
                opponents: list[dict[str, Any]] = []
                for participant in field if isinstance(field, list) else []:
                    if not isinstance(participant, dict) or participant is own or participant.get("itsHim") is True:
                        continue
                    label = str(participant.get("nomCheval") or "").strip()
                    if not label:
                        continue
                    position, disqualified, result_status = _result_position(participant.get("place"))
                    opponents.append({
                        "horse_name": label,
                        "position": position,
                        ...
                        "chrono_km_seconds": (
                            float(participant["reductionKilometrique"]) / 100
                            if isinstance(participant.get("reductionKilometrique"), (int, float))
                            else None
                        ),
                        "distance_m": participant.get("distanceParcourue"),
                    })
                row = {
                    "date": race_date,
                    "hippodrome": course.get("hippodrome"),
                    "code_course": course.get("nomPrix"),
                    "nom_course": course.get("nomPrix"),
                    "discipline": _text(course.get("discipline")),
                    "distance": course.get("distance"),
                    "position": own_position,
                    "disqualifie": own_dq,
                    "status_arrivee": own_status,
                    "reduction_km": (
                        float(own["reductionKilometrique"]) / 100
                        if isinstance(own.get("reductionKilometrique"), (int, float))
                        else None
                    ),
            ...
            if number is not None and horse_name:
                runners.append({"number": number, "horse_name": horse_name, "historique": history})
        return {
            "data": {"runners": runners},
            "meta": {
                "source": "PMU performances détaillées",
                "status": "ok" if any(item["historique"] for item in runners) else "no_performance_rows",
                "runners": len(runners),
            },
        }

    async def get_detailed_performances(self, day: date, code: str) -> dict[str, Any]:
        try:
            reunion, course = code.upper().split("C", 1)
            rnum = reunion.removeprefix("R")
        except ValueError as exc:
            raise ProviderError(f"Code course PMU invalide: {code}") from exc
        raw = await self._get(
            f"programme/{day.strftime('%d%m%Y')}/R{rnum}/C{course}/performances-detaillees/pretty",
            {"specialisation": "INTERNET"},
        )
        return self.normalize_detailed_performances(raw)
