...
def analysis_paragraph(
    ...
    """Produce a factual, deterministic horse-by-horse reading.

    The paragraph is generated only from fields already admitted by the
    independence firewall. It never mentions market position or outside tips.
    """
    ...
    if history:
        ...
        marks = []
        for h in recent:
            result = "disqualifié" if h.disqualified else (f"{h.position}e" if h.position else "non classé")
            context_bits = []
            if h.track:
            ...
            if h.going:
                context_bits.append(str(h.going))
            if h.chrono_km_seconds:
                context_bits.append(f"{h.chrono_km_seconds:.1f} s/km")
            if h.class_name:
                context_bits.append(str(h.class_name))
            context = f" ({', '.join(context_bits)})" if context_bits else ""
            marks.append(f"{result}{context}")
        evidence = f"dispose de {len(history)} performances documentées ; ses {len(recent)} références les plus récentes donnent " + ", ".join(marks) + "."
    elif runner.recent_form:
        evidence = f"a pour musique officielle disponible « {runner.recent_form} » ({len(music_observations)} résultats lisibles), mais l’historique détaillé manque encore pour vérifier chaque ligne."
    else:
        evidence = "a encore un historique détaillé insuffisant dans les sources chargées ; cette limite augmente mécaniquement l’incertitude."

    context_bits = []
    if race.discipline:
        context_bits.append(f"discipline {race.discipline}")
    if race.distance_m:
        context_bits.append(f"distance {race.distance_m} m")
    if race.start_type:
        context_bits.append(f"départ {race.start_type}")
    if runner.start_position is not None:
        context_bits.append(f"position de départ {runner.start_position}")
    if runner.weight_kg is not None:
        context_bits.append(f"poids {runner.weight_kg:g} kg")
    if race.going:
        context_bits.append(f"terrain {race.going}")
    if race.class_name:
        context_bits.append(f"catégorie {race.class_name}")
    if runner.handicap_value is not None:
        context_bits.append(f"valeur handicap {runner.handicap_value:g}")
    if runner.age is not None:
        context_bits.append(f"âge {runner.age} ans")
    if runner.sex:
        context_bits.append(f"sexe {runner.sex}")
    if runner.draw is not None:
        context_bits.append(f"corde/position {runner.draw}")
    if runner.ferrure:
        context_bits.append(f"ferrure {runner.ferrure}")
    if runner.equipment:
        context_bits.append(f"équipement {runner.equipment}")
    if runner.record_km_seconds:
        context_bits.append(f"record kilométrique {runner.record_km_seconds:.1f} s")
    if runner.earnings_eur is not None:
        context_bits.append(f"gains de carrière {runner.earnings_eur:,.0f} €".replace(",", " "))
    if race.surface:
        context_bits.append(f"surface {race.surface}")
    if runner.jockey_driver:
        context_bits.append(f"pilote {runner.jockey_driver}")
    if runner.trainer:
        context_bits.append(f"entraîneur {runner.trainer}")
    context = (" Les paramètres du jour connus sont : " + ", ".join(context_bits) + ".") if context_bits else ""
    ...
    volatility = "faible" if uncertainty <= 30 else "moyenne" if uncertainty < 60 else "forte"
    signals = (" Les signaux principaux sont : " + "; ".join(reasons[:4]) + ".") if reasons else ""
    return (
        f"{identity} {evidence}{context} {reading} {safety} avec une robustesse de {robust:.0f}/100. "
        f"Son potentiel caché atteint {hidden:.0f}/100 et sa volatilité est {volatility} ({uncertainty:.0f}/100). "
        f"Les notes indépendantes sont Performance {performance:.0f}, Placé {placed:.0f}, potentiel caché {hidden:.0f}, robustesse {robust:.0f} et volatilité {uncertainty:.0f}/100.{signals}"
    )
...
def score_race(race: Race, runners: list[Runner]) -> dict[int, ScoreCard]:
    ...
    for runner in active:
        ...
        if fam in ('trot_attele','trot_monte'):
            performance = (
                form*.20 + speed*.20 + aptitude*.13 + cls*.12 + prog*.10 +
                hidden*.09 + cons*.06 + wd*.04 + equip*.025 + line*.015 + value_signal*.02
            )
        else:
            performance = (
                form*.23 + aptitude*.16 + cls*.13 + prog*.11 + hidden*.10 +
                cons*.09 + wd*.07 + speed*.04 + equip*.015 + line*.015 + value_signal*.04
            )
        ...
        if line >= 66: reasons.append("Ligne objective confirmée par les adversaires")
        ...
        paragraph=analysis_paragraph(race,runner,hist,performance,placed,hidden,robust,uncertainty,reasons)
        ...
        output[runner.id] = ScoreCard(
            ...
            breakdown={
                ...
                "sample_size": _sample_count(hist, runner.recent_form), "history_rows": len(hist),
                "official_music_rows": len(_music_observations(runner.recent_form)), "principle": "own_performance_over_indirect_lines",
